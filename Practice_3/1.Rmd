...
#### How do the results dier between the training and the validation set?
...
Firstly, LDA is not interpretable because it doesn't apply subset selection. And logistic with best subset selection can be better if the selected subset is small. And Logistic with lasso and Nearest shrunk centroids are both very interpretable with small subset.

## Question 7

- a. Linlog >= GaussI. We train the binary classifiers via maximum likelihood and we are not sure p(y) is uniform. Linlog is better at binary classifcation.

- b. Can't tell With unconstained covariance matrix, it's hard to tell which one can perform better.

- c. Can't tell. It depends on the data set that including polynomial basis function expansion may be better or worse.

- d. GaussI <= Qualog, We train the binary classifiers via maximum likelihood and we are not sure p(y) is uniform. Qualog is better at binary classifcation.
