...
def separateByClass(dataset):
	...
	for i in range(len(dataset)):
		vector = dataset[i]
		if (vector[-1] not in separatedClass):
			separatedClass[vector[-1]] = []
		separatedClass[vector[-1]].append(vector)
...
def getNeighbors(trainingSet, testInstance, width):
	distances = []
	...
	for k in neighborsDic:
		if (k == "1"):
			for neighbor in neighborsDic[k]:
					diff1 = euclideanDistance(neighbor, testInstance, length)
			...
			for neighbor in neighborsDic[k]:
					diff0 = euclideanDistance(neighbor, testInstance, length)
...
def getResponse(prior, prob, testInstance):
	r = []
...
if __name__=="__main__":
    ...
    separatedClass = separateByClass(trainingSet)
    ...
    for x in range(len(testSet)):
    	neighbors = getNeighbors(trainingSet, testSet[x], width)
    	print("neighbors" + "\n")
    	print(neighbors)

    	neighborsClass = separateByClass(neighbors)
    	print("neighborsClass" + "\n")
    	print(neighborsClass)

    	prob = kernel(neighborsClass, testSet[x], width)

    	response = getResponse(prior, prob, testSet[x])
    	print("testSet[x] is:")
    	print(testSet[x])
